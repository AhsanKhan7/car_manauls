...
export default ({location, pageContext: {manual}}) => {
    const [manualPrompt, setManualPrompt] = useState('Download Manual');
    let decrementTimer;
    ...
    function unlockFile() {
        ...
        decrementTimer = 3;
        window.setTimeout(_ => {
            manualUnlock(manual.s3URL).then(unlockedUrl => {
                navigate('pdf/', {state: {url: unlockedUrl, s3URL: manual.s3URL, manual}});
            });
        }, decrementTimer * 1000);
        window.setTimeout(decrement);
    }

    function decrement() {
        setManualPrompt('loading');
    }

    return (
        <Layout siteTitle={`${manual.year} ${manual.make} ${manual.model} - Owner's manual PDF`} description={`Download or view the ${manual.year} ${manual.make} ${manual.model} PDF car owner’s manual.`}>
                    ...
                    <Row>
                        ...
                        <Col md={6} xs={12}>
                            <div className="product_d_right">
                                <h1>Download the <span className="dynamic-text">{manual.year} {manual.make} {manual.model}</span> PDF car owner’s manual</h1>

                                <div className="product_desc">
                                    <h2>
                                        The <span className="dynamic-text">{manual.year} {manual.make} {manual.model}</span> owner’s manual contains information on the operation and location of controls, a maintenance schedule and specific technical information like recommended fluid types, light bulb part numbers and electronic system controls. The PDF manual will help you troubleshoot common problems and operate your vehicle safely.
                                    </h2>
                                </div>
                                            ...
                                            <form action="#">
                                                <table>
                                                    <tbody itemScope itemType='https://schema.org/Car'>
                                                        {Object.keys(manual.carDBAPIdata).map((dataPoint, key) =>{
                                                            if(!mapped[dataPoint])
                                                                return (<></>);
                                                            ...
                                                            <tr key={key}>
                                                                ...
                                                                <td itemProp={manual.carDBAPIdata[dataPoint].semantic} >{manual.carDBAPIdata[dataPoint]._text}</td>
                                                            </tr>);
                                                        }
                                                        )}
                                                    </tbody>
                ...
                <StaticQuery
                    ...
                    render={(data) => {
                        let disqusConfig = {
                            ...
                            title: `${manual.make} Discussion`,
                        };
                        return (
                            <Container>
                                <h1>{manual.make} Discussion</h1>
